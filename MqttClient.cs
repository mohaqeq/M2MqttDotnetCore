...
using MqttUtility = M2Mqtt.Utility;
using System.IO;
using System.Linq;
using M2Mqtt;
using M2Mqtt.Net;
...
namespace M2Mqtt
{
    ...
    /// </summary>
        ...
        [Obsolete(
            "Use this ctor MqttClient(string brokerHostName, int brokerPort, bool secure, X509Certificate caCert) insted"
            )]
        ...
        public MqttClient(string brokerHostName, int brokerPort, bool secure, X509Certificate caCert,
            ...
            : this(
                brokerHostName, brokerPort, secure, caCert, clientCert, sslProtocol, userCertificateValidationCallback,
                null)
        {
        }
    ...
    // check security parameters
...
#if !(MF_FRAMEWORK_VERSION_V4_2 || MF_FRAMEWORK_VERSION_V4_3 || COMPACT_FRAMEWORK || WINDOWS_APP || WINDOWS_PHONE_APP)
            this.channel = new MqttNetworkChannel(this.brokerHostName, this.brokerPort, secure, caCert, clientCert,
                sslProtocol, userCertificateValidationCallback, userCertificateSelectionCallback);
#elif (WINDOWS_APP || WINDOWS_PHONE_APP)
        ...
        {
            return this.Connect(clientId, null, null, false, MqttMsgConnect.QOS_LEVEL_AT_MOST_ONCE, false, null, null,
                true, MqttMsgConnect.KEEP_ALIVE_PERIOD_DEFAULT);
        }
        ...
        public byte Connect(string clientId,
            ...
            string password)
        {
            return this.Connect(clientId, username, password, false, MqttMsgConnect.QOS_LEVEL_AT_MOST_ONCE, false, null,
                null, true, MqttMsgConnect.KEEP_ALIVE_PERIOD_DEFAULT);
        }
        ...
        {
            return this.Connect(clientId, username, password, false, MqttMsgConnect.QOS_LEVEL_AT_MOST_ONCE, false, null,
                null, cleanSession, keepAlivePeriod);
        }
        ...
        private void OnMqttMsgPublishReceived(MqttMsgPublish publish)
        {
            if (this.MqttMsgPublishReceived != null)
            {
                this.MqttMsgPublishReceived(this,
                    new MqttMsgPublishEventArgs(publish.Topic, publish.Message, publish.DupFlag, publish.QosLevel,
                        publish.Retain));
            }
        }
        ...
        private bool EnqueueInflight(MqttMsgBase msg, MqttMsgFlow flow)
        {
                ...
                (msg.QosLevel == MqttMsgBase.QOS_LEVEL_EXACTLY_ONCE))
            {
                lock (this.inflightQueue)
                {
                    ...
                    MqttMsgContextFinder msgCtxFinder = new MqttMsgContextFinder(msg.MessageId,
                        MqttMsgFlow.ToAcknowledge);
                    //MqttMsgContext msgCtx = (MqttMsgContext)this.inflightQueue.Get(msgCtxFinder.Find);
                    MqttMsgContext msgCtx =
                        (MqttMsgContext) this.inflightQueue.ToArray().FirstOrDefault(msgCtxFinder.Find);
    ...
    // update last message received ticks
                        ...
                        msgType =
                            (byte)
                                ((fixedHeaderFirstByte[0] & MqttMsgBase.MSG_TYPE_MASK) >> MqttMsgBase.MSG_TYPE_OFFSET);
                        ...
                        switch (msgType)
                        {
                            ...
                            case MqttMsgBase.MQTT_MSG_CONNACK_TYPE:
                                ...
                                throw new MqttClientException(MqttClientErrorCode.WrongBrokerMessage);
#else
                                this.msgReceived = MqttMsgConnack.Parse(fixedHeaderFirstByte[0],
                                    (byte) this.ProtocolVersion, this.channel);
#if TRACE
                            ...
                            case MqttMsgBase.MQTT_MSG_PINGRESP_TYPE:
                                ...
                                throw new MqttClientException(MqttClientErrorCode.WrongBrokerMessage);
#else
                                this.msgReceived = MqttMsgPingResp.Parse(fixedHeaderFirstByte[0],
                                    (byte) this.ProtocolVersion, this.channel);
#if TRACE
                                MqttUtility.Trace.WriteLine(TraceLevel.Frame, "RECV {0}", this.msgReceived);
#endif
                            ...
                            case MqttMsgBase.MQTT_MSG_SUBACK_TYPE:
                                ...
                                MqttMsgSuback suback = MqttMsgSuback.Parse(fixedHeaderFirstByte[0],
                                    (byte) this.ProtocolVersion, this.channel);
#if TRACE
                            ...
                            case MqttMsgBase.MQTT_MSG_PUBLISH_TYPE:
                                ...
                                MqttMsgPublish publish = MqttMsgPublish.Parse(fixedHeaderFirstByte[0],
                                    (byte) this.ProtocolVersion, this.channel);
#if TRACE
                            ...
                            case MqttMsgBase.MQTT_MSG_PUBACK_TYPE:
                                ...
                                MqttMsgPuback puback = MqttMsgPuback.Parse(fixedHeaderFirstByte[0],
                                    (byte) this.ProtocolVersion, this.channel);
#if TRACE
                            ...
                            case MqttMsgBase.MQTT_MSG_PUBREC_TYPE:
                                ...
                                MqttMsgPubrec pubrec = MqttMsgPubrec.Parse(fixedHeaderFirstByte[0],
                                    (byte) this.ProtocolVersion, this.channel);
#if TRACE
                            ...
                            case MqttMsgBase.MQTT_MSG_PUBREL_TYPE:
                                ...
                                MqttMsgPubrel pubrel = MqttMsgPubrel.Parse(fixedHeaderFirstByte[0],
                                    (byte) this.ProtocolVersion, this.channel);
#if TRACE
                            ...
                            case MqttMsgBase.MQTT_MSG_PUBCOMP_TYPE:
                                ...
                                MqttMsgPubcomp pubcomp = MqttMsgPubcomp.Parse(fixedHeaderFirstByte[0],
                                    (byte) this.ProtocolVersion, this.channel);
#if TRACE
                            ...
                            case MqttMsgBase.MQTT_MSG_UNSUBACK_TYPE:
                                ...
                                MqttMsgUnsuback unsuback = MqttMsgUnsuback.Parse(fixedHeaderFirstByte[0],
                                    (byte) this.ProtocolVersion, this.channel);
#if TRACE
                                ...
                                switch (msgContext.State)
                                {
                                    ...
                                    case MqttMsgState.SendUnsubscribe:
                                        ...
                                        if (msgContext.Flow == MqttMsgFlow.ToPublish)
                                        {
                                            ...
                                            timeout = (this.settings.DelayOnRetry < timeout)
                                                ? this.settings.DelayOnRetry
                                                : timeout;
...
#if TRACE
                                                        MqttUtility.Trace.WriteLine(TraceLevel.Queuing, "dequeued {0}",
                                                            msgReceived);
#endif
                                                    }
...
#if TRACE
                                                    MqttUtility.Trace.WriteLine(TraceLevel.Queuing, "processed {0}",
                                                        msgInflight);
#endif
                                                }
                                            }
                                                    ...
                                                    else
                                                    {
                                                        ...
                                                        if (msgInflight.Type == MqttMsgBase.MQTT_MSG_PUBLISH_TYPE)
                                                        {
                                                            ...
                                                            if ((this.session != null) &&
                                                                ...
                                                                (this.session.InflightMessages.Contains(msgContext.Key)))
#else
                                                                (this.session.InflightMessages.ContainsKey(
                                                                    msgContext.Key)))
#endif
                                                            {
                                                                this.session.InflightMessages.Remove(msgContext.Key);
                                                            ...
                                                            internalEvent = new MsgPublishedInternalEvent(msgInflight,
                                                                false);
                                    ...
                                    case MqttMsgState.WaitForPubrec:
                                        ...
                                        if (msgContext.Flow == MqttMsgFlow.ToPublish)
                                        {
                                            ...
                                            if ((msgReceived != null) &&
                                                (msgReceived.Type == MqttMsgBase.MQTT_MSG_PUBREC_TYPE))
                                            {
                                    ...
                                    case MqttMsgState.WaitForPubrel:
                                        ...
                                        if (msgContext.Flow == MqttMsgFlow.ToAcknowledge)
                                        {
                                            ...
                                            if ((msgReceived != null) &&
                                                (msgReceived.Type == MqttMsgBase.MQTT_MSG_PUBREL_TYPE))
                                            {
                                    ...
                                    case MqttMsgState.WaitForPubcomp:
                                        ...
                                        if (msgContext.Flow == MqttMsgFlow.ToPublish)
                                        {
                                            ...
                                            if ((msgReceived != null) &&
                                                (msgReceived.Type == MqttMsgBase.MQTT_MSG_PUBCOMP_TYPE))
                                            {
                                                     ...
                                                     (msgReceived.Type == MqttMsgBase.MQTT_MSG_PUBREC_TYPE))
                                            {
                                                ...
                                                if (msgReceived.MessageId == msgInflight.MessageId)
                                                {
                                                    lock (this.internalQueue)
                                                    {
                                                        ...
                                                        msgReceivedProcessed = true;
#if TRACE
                                                        MqttUtility.Trace.WriteLine(TraceLevel.Queuing,
                                                            "dequeued {0}", msgReceived);
#endif
    ...
    /// <param name="session">MQTT Client session to load</param>
        ...
        private ushort GetMessageId()
        {
            ...
            this.messageIdCounter = ((this.messageIdCounter%UInt16.MaxValue) != 0)
                ? (ushort) (this.messageIdCounter + 1)
                : (ushort) 1;
