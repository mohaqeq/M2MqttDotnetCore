...
// else other frameworks (.Net, .Net Compact, Mono, Windows Phone) 
        ...
        public override byte[] GetBytes(byte protocolVersion)
        {
            ...
            {
                // check topic length
                if ((this.topics[topicIdx].Length < MIN_TOPIC_LENGTH) ||
                    (this.topics[topicIdx].Length > MAX_TOPIC_LENGTH))
            ...
            if (protocolVersion == MqttMsgConnect.PROTOCOL_VERSION_V3_1_1)
                buffer[index++] = (MQTT_MSG_UNSUBSCRIBE_TYPE << MSG_TYPE_OFFSET) | MQTT_MSG_UNSUBSCRIBE_FLAG_BITS;
                    // [v.3.1.1]
            else
            {
